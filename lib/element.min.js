 ...
 * APPlICATION STATE
 * 
 * A global object that all modules have access too
 */
...
export const scriptHost = { server: (globalThis.Deno) ? true : false, client: (globalThis.Deno) ? false : true };
...
export function navigateTo (path) {
    if (path === undefined) globalThis.dispatchEvent(new Event('popstate'));
        ...
        if (typeof path != 'string') {
            console.warn('Provided path must be of type string:', path);
            return;
        }
...
export async function runAtServer(fn) {
...
export async function createComponent (element, ctx) {
    ...
    Object.defineProperties(element, {
        ...
        '_useState': {
            value: (obj) => {
                if (renderingStatus.atClient) {
                    if (element.hasAttribute('data-x-state')) {
                        Object.assign(_state, obj);
                        Object.assign(_state, JSON.parse(element.getAttribute('data-x-state')));
                        element.removeAttribute('data-x-state');
                    }
                }
                else {
                    Object.assign(_state, obj);
        ...
        '_hidden': {
            set: (value) => {
                ...
                if (value) {
                    element.classList.add('el-dsp-none');
                }
                else {
                    element.classList.remove('el-dsp-none');
                }
            },
            get: () => {
                return element.classList.contains('el-dsp-none');
            }
        }
    });

    if (element.hasAttribute('data-src')) {
        const module = await ctx.importModule(element.getAttribute('data-src'));
        element.setAttribute('data-is', module.componentName);
    }
    ...
    element._extend({
        ...
        add: async (id) => {
            ...
            if (!element._components[id]) {
                clone = element.querySelector('template').content.firstElementChild.cloneNode(true);
                clone.setAttribute("data-id", id);
                clone.setAttribute("data-parent", element.getAttribute("data-id"));
                element.appendChild(clone);
                await createComponent(clone, ctx);
        ...
        remove: (item) => {
            if (renderingStatus.loaded) {
                element.removeChild(element._components[item]);
        ...
        onclick: {
            set: (value) => {
                ...
                _onclick = (e) => { 
                    ...
                    if (value(e) === false) return;
                    if (element._href) navigateTo(element._href);
